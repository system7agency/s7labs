...
export function PageScripts() {
  useEffect(() => {
    ...
    const l1 = document.querySelector<HTMLElement>('.creator-lab .hero-title .line.l1')
    const l2 = document.querySelector<HTMLElement>('.creator-lab .hero-title .line.l2')
    ...
    document.querySelectorAll('.creator-lab .reveal').forEach((el) => revealIO.observe(el))
    ...
    const whyIO = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            const cards = e.target.querySelectorAll('[data-why]')
            cards.forEach((c, idx) => later(() => c.classList.add('in'), idx * 120))
    ...
    observers.push(whyIO)
    document.querySelectorAll('.creator-lab .why-grid').forEach((g) => whyIO.observe(g))

    /* ── Pipeline sequential lit ── */
    const pipeIO = new IntersectionObserver(
      (entries) => {
        entries.forEach((e) => {
          if (e.isIntersecting) {
            const steps = e.target.querySelectorAll('[data-step]')
            steps.forEach((s, idx) => later(() => s.classList.add('lit'), 200 + idx * 220))
