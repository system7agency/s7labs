...
export default function JobBriefPage() {
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      // time while the lead-save round-trips. On lead-save failure (bad email,
      // rate limit, etc.) revert to idle and surface the error.
      const label =
        inputMode === 'url'
      ...
      if (data.ok) {
        ...
        }).catch((err) => console.error('[job-brief] leads/complete fail', err))
      }
      setSubmitting(false)
    },
    [
