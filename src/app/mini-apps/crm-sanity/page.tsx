...
import {
  CrmSanityResult,
  buildCrmSanityPlainText,
  FORMAT_LABEL,
} from './components/CrmSanityResult'
...
export default function CrmSanityPage() {
  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      if (data.ok) {
        ...
        }).catch((err) => console.error('[crm-sanity] leads/complete fail', err))
      }
      setSubmitting(false)
    },
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      await navigator.clipboard.writeText(buildCrmSanityPlainText(result))
    } catch {
      const ta = document.createElement('textarea')
      ta.value = buildCrmSanityPlainText(result)
      ta.style.cssText = 'position:fixed;opacity:0'
      document.body.appendChild(ta)
      ta.select()
      document.execCommand('copy')
