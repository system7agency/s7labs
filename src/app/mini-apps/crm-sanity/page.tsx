  ...
  const handleSubmit = useCallback(
    async (e: FormEvent) => {
      ...
      if (!recordText.trim() || recordText.trim().length < 20) {
        setInputError('Paste at least a few CRM fields to analyse.')
        setShakeInput((k) => k + 1)
        return
      }
      ...
      try {
        const res = await fetch('/api/mini-apps/crm-sanity', {
          ...
          body: JSON.stringify({ record_text: recordText.trim() }),
        })
    ...
    [recordText, submitting, startLoadingAnimation, clearTimers]
  )
        ...
        <div className="panel-wrap">
          <div className="panel">
            ...
            <div className="panel-body">
              {/* IDLE */}
              <section className={`cs-state${appState === 'idle' ? 'active' : ''}`}>
                <div className="idle-label">CRM record data</div>
                <form noValidate onSubmit={handleSubmit} autoComplete="off">
                  <div className="textarea-field">
                    <label>
                      Paste field values (any format — key: value, CSV, JSON, free text)
                    </label>
                    <div
                      key={`r-${shakeInput}`}
                    ...
                    <div className="char-count">{recordText.length} chars</div>
                  </div>
              ...
              {/* LOADING */}
              <section className={`cs-state${appState === 'loading' ? 'active' : ''}`}>
              ...
              {/* ERROR */}
              <section className={`cs-state error-state${appState === 'error' ? 'active' : ''}`}>
